...
p$ccs_2050 <- filter(df$all,Variable%in%df$var$Variable,Year==2050) %>% 
    ...
    mytheme$theme_set1+theme(legend.position='right',strip.background=element_blank(),axis.text.x=element_text(angle=45,hjust=1))+
...
p$tmp1 <- plot_grid(p$emi_2050,p$emi_CO2_sec+theme(legend.position='none'),
                    nrow=1,axis='l',align='vh',rel_widths=c(1,1.3),labels=c('a','b'))
p$tmp2 <- plot_grid(p$ccs_2050,p$cdr_2050,p$l1,nrow=1,axis='tb',align='h',rel_widths=c(1,1,.3),labels=c('c','d',''))
p$tmp <- plot_grid(p$tmp1,p$tmp2,ncol=1,rel_heights=c(1,.8))
ggsave(filename='output/fig1.png',plot=p$tmp,width=200,height=150,units='mm',dpi=300,bg='white')
...
p$fin_ene_path <- filter(df$all,Variable=='Fin_Ene') %>% 
    ...
    ylim(0,13.5)+
    labs(title=NULL,x=NULL,y=expression(paste('Final energy (EJ ',yr^{`-1`},')')))+
    mytheme$theme_set1+theme(legend.position=c(.3,.4),legend.background=element_blank())+
    scale_color_manual(values=df$scen_col,name=NULL)+
    guides(color=guide_legend(title=NULL))
...
p$tmp1 <- plot_grid(p$fin_ene_path,
                    p$fec_2050+theme(axis.text.y=element_blank(),axis.ticks.y=element_blank(),axis.line.y=element_blank(),axis.title.y=element_blank()),
                    p$fin_ene_ele+theme(legend.position='none'),
                    nrow=1,rel_widths=c(1,.9,1),axis='tb',align='h',labels=c('a','','b'))
p$fec_sec <- plot_grid(p$fec_ind+theme(legend.position='none'),p$fin_ene_ind_ele+theme(legend.position='none'),
                       ...
                       nrow=1,rel_widths=c(1,.7,1,.7,1,.7),axis='tb',align='h',labels=c('c','','d','','e',''))
p$tmp <- plot_grid(p$tmp1,p$fec_sec,ncol=1,rel_heights=c(1,.9),labels=c('a',''))
ggsave(filename='output/fig2.png',plot=p$tmp,bg='white',width=200,height=140,units='mm',dpi=300)
...
p$tmp <- plot_grid(p$sec_ele_path+theme(legend.position='none'),p$sec_ele_2050+theme(axis.title.y=element_blank(),axis.text.y=element_blank(),axis.line.y=element_blank(),axis.ticks.y=element_blank()),
                   p$sec_hyd_2050,nrow=1,rel_widths=c(1,1.3,1.5),axis='tb',align='h',labels=c('a','','b'))
ggsave(filename='output/fig3.png',plot=p$tmp,width=200,height=85,units='mm',dpi=300)
...
df$var <- tribble(~Variable,~Legend,~Color,
                  ...
                  'Prm_Ene_Sec_Ene_Trd','Secondary energy\nimport','thistle2')
leg <- as.character(df$var$Legend); names(leg) <- as.character(df$var$Variable)
col <- as.character(df$var$Color); names(col) <- as.character(df$var$Variable)
...
p$imp2 <- filter(df$all,Variable%in%df$var$Variable) %>% 
    ...
    mytheme$theme_set1+theme(legend.position='right',axis.text.x=element_text(angle=45,hjust=1),axis.text.y=element_blank(),axis.line.y=element_blank(),axis.ticks.y=element_blank())+
    scale_fill_manual(values=col,labels=leg,name=NULL)
...
p$tmp1 <- plot_grid(p$imp_path+theme(legend.position=c(.25,.3),legend.background=element_blank()),
                    p$prm_2050+theme(legend.position='none'),
                    nrow=1,rel_widths=c(1,.7),axis='tb',align='h',labels=c('a',''))
...
p$tmp1 <- plot_grid(p$imp_cos_path+theme(legend.position='none'),p$imp2+theme(legend.position='none'),
                    nrow=1,rel_widths=c(1,.55),axis='tb',align='h',labels=c('b',''))
p$tmp3 <- plot_grid(p$tmp1,p$l1,ncol=1,rel_heights=c(1,.2))
p$tmp <- plot_grid(p$tmp2,p$tmp3,nrow=1,rel_widths=c(1,.85))
ggsave(filename='output/fig4.png',plot=p$tmp,bg='white',width=200,height=110,units='mm',dpi=300)
...
p$tmp <- plot_grid(p$car_pri+theme(legend.position=c(.3,.75)),p$cum_inv+theme(legend.position='none'),p$inv_2050,
                   nrow=1,axis='tb',align='h',labels=c('a','b','c'),rel_widths=c(1,.8,1.3))
ggsave(filename='output/fig5.png',plot=p$tmp,width=200,height=80,units='mm',dpi=300)
